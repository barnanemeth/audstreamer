...
final class DefaultSocketUpdater {
    ...
    var isThisDeviceCurrent: AnyPublisher<Bool, Error> {
        Publishers.CombineLatest(socket.getActiveDevice(), socket.getDeviceList())
            .map { DeviceHelper.isDeviceIDCurrent($0) || $1.allSatisfy { $0.id == DeviceHelper.deviceID } }
            .eraseToAnyPublisher()
    }
}
...
extension DefaultSocketUpdater: SocketUpdater {
