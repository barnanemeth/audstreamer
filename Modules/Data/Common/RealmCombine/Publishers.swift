...
enum RealmPublishers {
    ...
    static func collection<Output: NotificationEmitter>(from collection: Output,
        ...
        -> AnyPublisher<Output, Error> {
            ...
            return RealmPublisher<Output, Error>(initialValue: initialValue) { subscriber in
                return collection.observe(keyPaths: keyPaths, on: nil) { changeset in
                    let value: Output
    ...
    static func array<Output: NotificationEmitter>(from collection: Output,
        ...
        -> AnyPublisher<[Output.ElementType], Error> {
            ...
            return RealmPublishers.collection(from: collection, keyPaths: keyPaths, synchronousStart: synchronousStart)
            .map { $0.toArray() }
            .eraseToAnyPublisher()
    }
    ...
    static func changeset<Output: NotificationEmitter>(from collection: Output,
        ...
        -> AnyPublisher<(AnyRealmCollection<Output.ElementType>, RealmChangeset?), Error> {
            ...
            return RealmPublisher<(AnyRealmCollection<Output.ElementType>, RealmChangeset?), Error>(initialValue: initialValue) { subscriber in
                return collection.toAnyCollection().observe(keyPaths: keyPaths, on: nil) { changeset in
                    switch changeset {
