...
extension DefaultRemotePlayer {
    private func initializeNowPlayingInfo() {
        nowPlayingInfoCenter.nowPlayingInfo = [
            ...
            Constant.artworkKey: MPMediaItemArtwork(
                ...
                requestHandler: { _ in Asset.Images.logoLarge.image }
            ),
    ...
    private func fetchArtwork(for item: NowPlayable) -> AnyPublisher<UIImage, Error> {
        guard let imageURL = item.imageURL else {
            return Just(Asset.Images.logoLarge.image).setFailureType(to: Error.self).eraseToAnyPublisher()
        }
        return ImagePipeline.shared.imagePublisher(with: imageURL)
            .map { $0.image }
            .catch { _ in Just(Asset.Images.logoLarge.image).setFailureType(to: Error.self).eraseToAnyPublisher() }
            .eraseToAnyPublisher()
    }
}
