...
final class DownloadWidget: UIView {
...
extension DownloadWidget {
    ...
    private func setupErrorImageView() {
        ...
        NSLayoutConstraint.activate([
            errorImageView.topAnchor.constraint(equalTo: topAnchor, constant: 2),
            errorImageView.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -2),
            errorImageView.leadingAnchor.constraint(equalTo: leadingAnchor, constant: .zero),
            errorImageView.widthAnchor.constraint(equalTo: heightAnchor)
        ])
    }

    private func setupTitleLabel() {
        titleLabel.text = L10n.downloading
...
extension DownloadWidget {
    private func update(with event: DownloadWidgetState, animationCompletion: @escaping (() -> Void)) {
        switch event {
        case let .inProgress(progress, items):
            updateViews(isErrorOccurred: false)
            titleLabel.text = L10n.downloading
            updateSubtitleText(progress: progress, items: items, isFinished: false)
            progressIndicatorView.currentProgress = progress.fractionCompleted * Constant.finishedAnimationFrame
        case let .finished(items):
            updateViews(isErrorOccurred: false)
            updateSubtitleText(progress: nil, items: items, isFinished: true)
        ...
        case let .error(error):
            updateViews(isErrorOccurred: true)
            titleLabel.text = L10n.error
            subtitleLabel.text = error.localizedDescription
            progressIndicatorView.currentProgress = .zero
            DispatchQueue.main.asyncAfter(deadline: .now() + Constant.errorDelay, execute: animationCompletion)
        }
    }
    ...
    private func updateSubtitleText(progress: Progress?, items: [Downloadable], isFinished: Bool) {
        if items.count == 1 {
            subtitleLabel.text = items.first?.title ?? ""
        } else if isFinished {
            subtitleLabel.text = L10n.downloadingEpisodesCountPercentage(items.count, 100)
        } else if let progress = progress {
            let percentage = Int(progress.fractionCompleted * 100)
            subtitleLabel.text = L10n.downloadingEpisodesCountPercentage(items.count, percentage)
        }
    }
}
